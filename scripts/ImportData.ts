...
const csvFilePath = __dirname + '/../../cleaning/data-clean.csv';
...
async function findOrCreateProduct(row: CsvRow, categoryId: number, quantityUnitId: number) {
  ...
  if (!product) {
    product = await db.product.create({
      data: { name: row.productName, genericName: row.genericName, categoryId, baseUnitId: quantityUnitId },
    });
  }
  return product;
}
